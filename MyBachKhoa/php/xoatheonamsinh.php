<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        if(empty($errors))
        {
                ...
                $query2="SELECT count(user) from tblinfo where ngaysinh like '$namsinh%'";
                $result2=mysqli_query($dbc,$query2) or die ("Query: $query \n <br> Mysql_error: ".mysqlli_error($dbc));
                list($count)=mysqli_fetch_array($result2);
                
                $query="SELECT user from tblinfo where ngaysinh like '$namsinh%'";
                ...
                {
                    $message ="<p class='required'>Năm sinh không tồn tại trong hệ thống. Vui lòng kiểm tra lại!!!</p>";
                }
...
<head>
    ...
    <title>Xóa User - Theo năm sinh</title>
                    ...
                    <div class="form">
                        ...
                        if(isset($message))
                        {
                            ...
                            <div class="form-group">
                                <label for="">Nhập năm sinh: </label>
                                <input value="<?php if(isset($_POST['namsinh'])) echo $_POST['namsinh'] ; ?>"
                                    name="namsinh" type="text" class="form-control" placeholder="Nhập năm sinh">
                                <?php
                                if(isset($errors) && in_array('namsinh',$errors))
                                {
                                    echo "<p class='required'>Bạn chưa nhập năm sinh</p>";
                                }
                            ?>
                            </div>
